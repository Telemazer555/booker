...
class TestBookingScenariosAsync:
    ...
    async def test_as_get_and_verify_items_exist_aiohttp_httpx(self, as_item_scenarios):
        tasks = [as_item_scenarios.as_get_and_verify_items_exist() for _ in range(2)]
        await asyncio.gather(*tasks)

    @pytest.mark.asyncio
    async def test_as_create_item_and_delete(self, as_item_scenarios):
        # booking_data_create = BookingResponseData()
        tasks = [as_item_scenarios.as_create_item_and_immediately_delete(item_data=BookingResponseDataModel()) for _ in
                 range(6)]
        await asyncio.gather(*tasks)
    ...
    async def test_as_update_and_get_and_delete(self, as_item_scenarios):
        ...
        tasks = [as_item_scenarios.as_update_item_and_verify_changes_and_delete(item_data=BookingResponseDataModel(),
                                                                                upd_item_data=BookingResponseDataModel()) for _
                 in range(1)]
        await asyncio.gather(*tasks)
